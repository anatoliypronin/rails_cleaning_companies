...
class CompanyAuthorization extends React.Component {
  ...
  handleSubmitPhone(event) {
    ...
    fetch('/api/v1/session_company', {
      method: 'POST',
      headers: {'Content-type': 'application/json'},
      body: JSON.stringify({'company': {'phone_number': this.state.phone}})
    })
  ...
  handleSubmitSmsCode(event) {
    ...
    fetch('/api/v1/session_company', {
      method: 'POST',
      headers: {'Content-type': 'application/json'},
      body: JSON.stringify({'company': {'phone_number': this.state.phone, 'sms_code': this.state.sms_code}})
    })
    .then(response => {if (response.ok) {window.location.href = '/company/profile'}})
  }
